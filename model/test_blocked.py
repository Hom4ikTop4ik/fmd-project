...
def delaunay_to_obj(points, filename="output.obj"):
    '''
...
def visualize_delaunay(points, img):
    ...
    for i, p in enumerate(img_points):
        ...
        val = points[i, 2] - 2.5
        cv2.circle(img, tuple(p), 6, (lev, 0, 0), -1)
...
def test():
    ...
    out = cv2.VideoWriter('output.mp4', fourcc, 15.0, (432,432)) #имя файла, кодек, fps, размер кадра
    ...
    while True:
        ...
        imgtens = frame.permute(2, 0, 1)[:, 0:432, 200:632].unsqueeze(0)
        print(imgtens.shape)
        predict = det1(imgtens)
        predict = mypca.decompress(predict).reshape(-1, 3, 68).permute(0, 2, 1)
        ...
        delaunay_to_obj(predict.squeeze(), f'anim/facemesh{frameid:04d}.obj')
