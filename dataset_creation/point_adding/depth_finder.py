...
def find_depth(face, coordsFromImageRaw,
              lEyeCornerIdObj, lEyeCornerIdImg,
              rEyeCornerIdObj, rEyeCornerIdImg,
              upNoseIdObj, upNoseIdImg, depth_id_list,
              accuracy = 40,
            debug:bool = False):
    '''
    coordsFromImageRaw should be list of np.arrays of 2 elements each.
    depth_id_list is list of indexes of facemesh points that we need to extract depth from.
    '''
    ...
    camDisplay = []
    for vect in vects:
        disvec = calcCameraBasedCoords(angleHor, angleVer, camDist, vect)
        depths.append(disvec[1])
        if debug:
    ...
    return [depths[i] for i in depth_id_list]